...
def promptOverwrite(dir):
	msg = HTML(f"<i><b>{dir}</b></i> already exists in this directory. <b>Overwrite {dir}?: </b>")
	p = SelectionPrompt()
...
def runCommand(cmd, shell=False):
	return subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=shell)
