...
def main(args, env=os.environ):
    ...
    parser_a.add_argument('-f', '--force', default=False, action='store_true', help='Force initialize Chinstrap project in the current directory. Be careful, this will potentioally overwrite files that exist in the directory.')
    parser_a.set_defaults(func=chinstrapInitialize)

    parser_b = subparsers.add_parser('config', help='Verify Chinstrap configuration')
    parser_b.add_argument('-n', '--network', default='development', help='Select the configured network')
    parser_b.set_defaults(func=chinstrapConfigVerification)

    parser_c = subparsers.add_parser('networks', help='List currently available test networks')
    ...
    parser_d.add_argument('-c', '--contract', help='Contract to compile. If not specified, all the contracts are compiled')
    parser_d.add_argument('-l','--local', default=False, action="store_true", help="Use compiler from host machine. If not specified, Docker image is used")
    parser_d.add_argument('-e', '--entrypoint', default='main', type=str, help='Entrypoint to use when compiling Ligo contracts. Default entrypoint is main')
    ...
    parser_e.add_argument('-f','--force', default=False, action="store_true", help="Force update compilers")
    parser_e.add_argument('-l','--local', default=False, action="store_true", help="Install on local machine. If not specified, Docker is used")
    parser_e.add_argument('-c','--compiler', type=Compilers, default=Compilers.all, choices=list(Compilers), help="Installs selected compilers")
    parser_e.set_defaults(func=chinstrapInstallCompilers)

    parser_f = subparsers.add_parser('create', help='Helper to create new contracts, originations and tests')
    parser_f.add_argument('-t', '--type', type=CreateOptions, choices=list(CreateOptions), required=True, help='The type of the item to create')
    parser_f.add_argument('-n', '--name',  required=True, help='The name of the item to create')
    parser_f.set_defaults(func=chinstrapCreate)

    parser_g = subparsers.add_parser('templates', help='Download templates provided by SmartPy')
    parser_g.set_defaults(func=chinstrapTemplates)

    parser_h = subparsers.add_parser('test', help='Run pytest/smartpy/ligo tests')
    parser_h.add_argument('-t', '--test', help='Test to run. If not specified, all the tests are executed')
    parser_h.add_argument('-l','--local', default=False, action="store_true", help="Run tests on host machine. If not specified, Docker image is preferred")
    parser_h.add_argument('-e', '--entrypoint', default='main', type=str, help='Entrypoint to use when compiling Ligo contracts. Default entrypoint is main')
    parser_h.set_defaults(func=chinstrapRunTests)

    parser_i = subparsers.add_parser('sandbox', help='Start a Tezos local sandbox')
    parser_i.add_argument('-o', '--port', default=20000, help="Tezos local sandbox's RPC Port")
    parser_i.add_argument('-i', '--initialize', default=False, action="store_true", help="Initialize Tezos sandbox")
    parser_i.add_argument('-d', '--detach', default=False, action="store_true", help="Start the Tezos sandbox and detach")
    parser_i.add_argument('-s', '--stop', default=False, action="store_true", help="Stop the currently running Tezos sandbox")
    parser_i.add_argument('-c', '--num-of-accounts', default=10, type=int, help="Number of accounts to bootstrap on Tezos sandbox")
