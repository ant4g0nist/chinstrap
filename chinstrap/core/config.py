...
class Config:
    ...
    def loadPrivateKey(self, key):
        try:
            ...
            Helpers.fatal(f"Failed to connect to {self.network.host}. Try again in sometime!")

        Helpers.printFormatted(f"Loaded wallet <ansiyellow><b>{wallet.key.public_key_hash()}</b></ansiyellow>. Balance: <ansigreen><b>{wallet.balance()}</b></ansigreen>\n")
